...
export default function MatchesPage() {
  ...
  const { request, matches, loading } = useMatches(params.id as string)
  ...
  return (
    <main className="min-h-screen bg-background pb-20 px-6 max-w-lg mx-auto">
      ...
      <div className="mb-6">
        ...
        {request && <p className="text-sm text-muted-foreground mt-1 font-medium">{request.title}</p>}
      </div>
      ...
      {loading ? (
        ...
        <Card variant="neo-elevated" className="border-none rounded-[2rem] p-8 text-center flex flex-col items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Tente ampliar o raio de busca ou mudar a categoria.</p>
          </div>
        </Card>
      ) : (
        <div className="space-y-4">
          {matches.map((prof) => (
            <Card key={prof.id} variant="neo-elevated" className="border-none rounded-[2rem] p-4 hover:translate-y-[-2px] transition-transform cursor-pointer group">
              <div className="flex gap-4 items-start">
                <div className="w-14 h-14 neo-inset rounded-2xl flex items-center justify-center text-2xl bg-background/50 shrink-0">🔧</div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs font-medium text-muted-foreground">
                    <div className="flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-primary" />{prof.distance_km?.toFixed(1)} km
                    </div>
                    {prof.hourly_rate_cents && (
                      <div className="flex items-center gap-1">
                        <DollarSign className="w-3 h-3 text-primary" />{formatCurrency(prof.hourly_rate_cents)}/h
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-muted/20 flex gap-2">
                <Button variant="neo-elevated" className="flex-1 h-10 rounded-xl text-xs font-bold text-primary"
