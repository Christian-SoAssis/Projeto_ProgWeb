...
async def analyze_request_task(ctx: Dict[str, Any], request_id: str) -> str:
    ...
    """
    with tracer.start_as_current_span("analyze_request_task") as span:
...
async def process_payouts_task(ctx: Dict[str, Any]) -> str:
    ...
    """
    with tracer.start_as_current_span("process_payouts_task") as span:
        ...
        async with async_session_maker() as session:
            ...
            await session.commit()
        span.set_attribute("payouts.processed_count", count)
...
async def check_disputes_deadline_task(ctx: Dict[str, Any]) -> str:
    ...
    """
    with tracer.start_as_current_span("check_disputes_deadline_task") as span:
        ...
        async with async_session_maker() as session:
            ...
            await session.commit()
        span.set_attribute("disputes.escalated_count", count)
...
async def run_data_retention_policy_task(ctx: Dict[str, Any]) -> str:
    ...
    """
    with tracer.start_as_current_span("run_data_retention_policy_task") as span:
        ...
        async with async_session_maker() as session:
            ...
            await session.commit()
        span.set_attribute("retention.anon_count", stats.get("anon_count", 0))
        span.set_attribute("retention.warned_count", stats.get("warned_count", 0))
...
async def log_matching_event_task(ctx: Dict[str, Any], events: list) -> str:
    ...
    """
    with tracer.start_as_current_span("log_matching_event_task") as span:
        span.set_attribute("matching.events_count", len(events) if events else 0)
